...
def draw_card(out: Path, headline: str, kicker: str = "", big: str = "", big_label: str = "",
    ...
    d.text((pad + d.textlength(label, font=mf), H - pad - 26), "POKE", font=mf, fill=HOT)
    if credit:
        cf = _font(BODY, 17)
        t = credit[:74]
...
if __name__ == "__main__":
    ...
    ch = jload(BUILD / "api" / "charts.json")
    lc = jload(BUILD / "api" / "coverage-langs.json")
    corpus = jload(BUILD / "api" / "corpus.json")
    terms = {t["th"]: t for t in corpus.get("terms", [])}

    # A share card is a finding, not a footnote. Lead with the number people came for.
    PAGES = [
        ("index", "Who is marked, who does the marking, and where on the body", "Hand Poke",
         str(ch["traditions"]), "traditions of marking skin by hand", "sak-kha-lai"),
        ("legs", "One custom, four peoples, and a border in the wrong place", "The leg zone",
         "3", "countries the zone crosses", "htoe-kwin"),
        ("clothing", "Five languages reach for the same comparison", "The tattoo is clothing",
         str(len(ch["clothing"])), "traditions whose sources say so", "tatau"),
        ("methods", "Poke, tap, stitch, cut — and the machine that renamed them all",
         "Four hands", "4", "ways a hand marks skin", "batok"),
        ("map", "Every tradition this site holds, on an equal-area map", "The map",
         str(len(ch["countries"])), "countries", "kakiniit"),
        ("timeline", "From the oldest marked skin to the newest studio", "Timeline",
         "5,300", "years, with the break drawn", "otzi"),
        ("body", "Skin clothing covers, and skin it does not", "Where the marks go",
         str(ch["open_skin"]), "marks on skin nothing covers", "chin-ming"),
        ("words", "Prick, strike, write, pattern", "The words",
         str(len(ch["words"])), "words for the same act", "sak-yant"),
        ("coverage", "A count of attention, not of importance", "Who gets written about",
         f'{lc["with_own_article"]}/{lc["traditions"]}',
         "have an article about the tattooing", "whang-od"),
        ("traditions", "Living, revived, thinning, dormant", "The traditions",
         str(ch["traditions"]), "counted here", "mentawai-titi"),
        ("counted", "Every number, with the thing it was counted from", "Counted",
         str(terms.get("สักขาลาย", {}).get("pages", 0)),
         f'pages of {corpus.get("corpus", {}).get("pages_transcribed", 0):,} name the leg tattoo',
         "murals"),
    ]
    n = 0
    ...
    for node in nodes:
        ...
        if not p:
            continue
        pr = node.get("practice") or {}
        big = ""
        label = ""
        if pr.get("method"):
            big = {"poke": "poke", "tap": "tap", "stitch": "stitch", "cut": "cut",
                   "mixed": "hand"}.get(pr["method"], "")
            label = (pr.get("status") or "")
